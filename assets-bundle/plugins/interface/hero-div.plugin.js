...
(function () {
  ...
  function isBoardTabSelected() {
    const boardTab = document.querySelector(
      'a[title="Board"].selected, a[href="#/"].selected, [class*="nav-tab-button-container"].selected[href="#/"]',
    );
    if (boardTab) return true;
    if (!isBoardPage()) return false;
    return Boolean(
      document.querySelector('a[title="Board"][href="#/"], a[href="#/"][title="Board"]'),
    );
  }
  ...
  function isBoardPage() {
    ...
    return path === "/" || path === "" || path.startsWith("/board");
  }
  ...
  function createHeroDirect() {
    ...
    const shouldResetScroll =
      !window.__stremioCustomScrollRestoreActive &&
      !window.__stremioCustomHeroSeenThisVisit;
    ...
    window.__stremioCustomHeroSeenThisVisit = true;
  ...
  async function addHeroDiv() {
    ...
    const boardElementsReady = await waitForBoardElements(12000);
  ...
  function handleNavigation() {
    ...
    if (!shouldShow && heroExists) {
      heroExists.remove();
      resetHeroState();
      window.__stremioCustomHeroSeenThisVisit = false;
      return;
    }

    if (shouldShow && !heroExists) {
      if (window.__stremioCustomScrollRestoreActive) {
        setTimeout(() => addHeroDiv(), 2800);
      } else {
